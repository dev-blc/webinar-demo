...
const TabsList = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={`inline-flex items-center justify-center rounded-md p-1 text-muted-foreground data-[orientation=horizontal]:space-x-1 data-[orientation=vertical]:space-y-1 bg-muted ${className || ""}`}
      {...props}
    />
  ),
)
